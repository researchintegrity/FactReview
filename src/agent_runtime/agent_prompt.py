...
def _build_review_annotator_prompt(
    *,
    ...
    return (
        ...
        "  paper_metadata -> overview_figure -> core_claims -> claim_evidence_matrix -> paper_summary -> strengths -> weaknesses -> experiments.\n"
...
def _build_fact_review_extractor_prompt(
    *,
    ...
    return (
        ...
        f"- file_id: {source_file_id}\n"
        f"- file_name: {source_file_name}\n"
        "- Always operate on this paper only.\n"
        "\n"
        "Primary objective:\n"
        "- Extract all important manuscript-grounded information that can be reliably supported by the paper text and figures.\n"
        "- Do not invent missing items. If a field is absent from the manuscript, write `Not found in manuscript`.\n"
        "- Do not perform reviewer scoring, acceptance recommendation, status labeling, or template-only administrative summaries.\n"
        "- Do not reproduce non-manuscript evaluation widgets, dashboard fields, or template-only control blocks.\n"
        "- Do not add extra sections, extra analysis blocks, or extra background notes beyond the required output schema.\n"
        "- Focus first on the exact requested extraction fields only.\n"
        "\n"
        "Extraction-only operating rules:\n"
        "- Prefer manuscript evidence over inference.\n"
        "- Default to zero retrieval calls: extract directly from the provided `[Paper Markdown]` in this prompt.\n"
        "- Do not run `pdf_search`/`pdf_read_lines` unless a critical field cannot be found in `[Paper Markdown]`.\n"
        "- Use `mcp_status_update` only for concise progress updates.\n"
        "- Use the provided Semantic Scholar retrieval block for objective related-work comparison in section 2; it was retrieved by querying the current paper title against Semantic Scholar.\n"
        "- Do not use reviewer-style annotations as the main output channel.\n"
        ...
        "1. `metadata`\n"
        "2. `technical_positioning`\n"
        "3. `claims`\n"
        "4. `summary`\n"
        "5. `experiment`\n"
        "\n"
        "Section content contract:\n"
        "- `1. Metadata`: markdown bullet list with exactly these fields: `Title`, `Task`, `Code`. If code is absent, write `Not found in manuscript`.\n"
        "- `2. Technical Positioning`: output one brief figure caption line, then one niche-positioning matrix table.\n"
        "- `2. Technical Positioning`: figure caption must be abbreviated and concise (prefer one sentence; <= 32 words if possible).\n"
        "- `2. Technical Positioning`: do not output markers like `[Figure Placeholder]`, `Overview Figure`, or `Overview Figure Page`.\n"
        "- `2. Technical Positioning`: run 3-step logic strictly:\n"
        "  Step 1: from Introduction/Related Work in manuscript, extract declared baselines and author self-positioning.\n"
        "  Step 2: use `[Semantic Scholar Retrieval]` papers as the objective top related works returned for the current paper title.\n"
        "  Step 3: perform gap analysis between declared set and objective set.\n"
        "- `2. Technical Positioning`: if `[Semantic Scholar Retrieval]` has `success: false` or empty `papers`, do NOT invent external papers; output `Objective retrieval unavailable` and skip R-columns expansion.\n"
        "- `2. Technical Positioning`: table must use fixed first two columns exactly `Research domain | Method`.\n"
        "- `2. Technical Positioning`: row semantics are strict: `Research domain` = field/subfield category or ecological niche; `Method` = concrete model/method/paper short name.\n"
        "- `2. Technical Positioning`: external related works must appear first; add `This Work` as the final row only.\n"
        "- `2. Technical Positioning`: self-row is strict: final row first cell must be exactly `This Work`; final row second cell must be the method proposed by this paper.\n"
        ...
        "- `2. Technical Positioning`: cells in niche-dimension columns must use only `√` or `×`; for the `This Work` row, use `√` only if the manuscript explicitly addresses that dimension, otherwise use `×`.\n"
        ...
        "- `3. Claims`: provide exactly 3 most important claims only.\n"
        ...
        "- `3. Claims`: `Evidence` must quote/paraphrase the exact manuscript support for the claim, including table/figure/equation/section anchors where available.\n"
        ...
        "- Under `Main Result`, include a `Location:` line first, then provide a SINGLE markdown table consolidating ALL main experimental results; use fixed columns exactly `Task | Dataset | Metric | Best Baseline | Paper Result | Difference (Δ)`; never split results into multiple tables or output column headers as data rows.\n"
        "- `Main Result` inclusion rule: include only tables where rows compare the proposed method against external baselines/SOTA; exclude any table whose rows compare internal variants, components, or hyperparameter settings.\n"
        "- Under `Ablation Result`, include a `Location:` line first indicating where ablation is reported and on which dataset(s).\n"
        "- `Ablation Result` source rule: extract ONLY from sections the manuscript explicitly identifies as ablation study or analysis (e.g., sections titled `Ablation Study`, `Ablation Analysis`, `Component Analysis`, `Analysis`); do NOT convert main result rows into ablation rows.\n"
        "- Under `Ablation Result`, output one markdown table per distinct ablation experiment group (a group shares the same task, dataset, metric, and full-model reference); use fixed columns exactly `Ablation Dimension | Configuration | Full Model | Paper Result | Difference (Δ)`.\n"
        "- Each ablation table must begin with an `Optimal setup` anchor row: `Ablation Dimension` = `Optimal setup`, `Configuration` = the manuscript's full/best-complete configuration name, `Full Model` = `Paper Result` = the optimal model's reported score, `Difference (Δ)` = 0.\n"
        "- Every subsequent row in each ablation table changes exactly one design variable from the optimal setup; `Ablation Dimension` = the axis being changed (e.g., `Encoder architecture`); `Configuration` = the variant name.\n"
        "- `Ablation Result`: if no section explicitly identified as ablation exists in the manuscript, output one placeholder table with exactly one row: `Ablation Dimension` = `Optimal setup`, `Configuration` = `Not found in manuscript`, `Full Model` = `Not found in manuscript`, `Paper Result` = `Not found in manuscript`, `Difference (Δ)` = 0.\n"
        "- Do not output any `Note:` line in `5. Experiment`.\n"
        "- If a table cannot be fully supported by the manuscript, keep the table but fill missing cells with `Not found in manuscript`.\n"
        "- Do not output any `Appendix` heading, appendix subsection, or appendix-only table in the final report.\n"
        "- Do not add any extra section beyond the five sections above.\n"
        "- Style rule: all sections except `4. Summary` must use concise fact-only wording; short phrases are preferred over long prose.\n"
        "- Style rule: avoid rhetorical language, filler, and speculation; write only manuscript-grounded facts.\n"
        "- Writing rule (strict factuality): every field, sentence, and table cell must be directly supported by the manuscript; if not supported, write `Not found in manuscript`.\n"
        "- Writing rule (conciseness): for sections `1/2/3/5`, use short academic phrasing; prefer noun phrases or one-sentence statements.\n"
        "- Writing rule (anti-verbosity): avoid long explanations, marketing language, and repeated wording; keep each claim/evidence item minimal and precise.\n"
        "- Writing rule (style): use objective academic language, avoid subjective judgments, and avoid unsupported causal statements.\n"
        "- Writing rule (template safety): never copy example/demo/template content; always extract from the current manuscript.\n"
        "\n"
        "Submission protocol:\n"
        "- Submit exactly one section per `review_final_markdown_write` call.\n"
        "- After each call, inspect `next_required_section` and submit the next required section immediately.\n"
        "- Stop only when the tool returns `status='ok'` or `task_completed=true`.\n"
        "\n"
